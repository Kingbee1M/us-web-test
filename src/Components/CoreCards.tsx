...
const CoreCards = ({ core }: CoreCardsProps) => {
    ...
    if (core.value) {
        return (
            ...
            ref={ref}
            key={core.id}
                className='bg-white w-100 sm:w-150 relative group'
                ...
                {/* shadow */}
                <div className='bg-[#D582E3] w-full h-[85%] absolute -bottom-4 left-6 rotate-[4deg] rounded-lg group-hover:rotate-[-4deg] transition-transform duration-500 ease-out'></div>
                ...
                <div className=" relative z-10 w-full h-full rounded-lg border-2 border-[#aa96da] overflow-hidden bg-white">
                    <div className='w-full h-full text-xl flex flex-col gap-3 z-30 p-3 pb-5 relative rounded-lg border-2 border-solid border-[#aa96da] hover:bg-pink'>
                        ...
                        <div>
                            ...
                            <ul className='pl-8 flex flex-col gap-3 text-xl'>
                    ...
                    <div className=" relative z-10 w-full h-full rounded-lg border-2 border-[#aa96da] overflow-hidden bg-white">
                        ...
                        <div>
                            ...
                            {Object.entries(core.industries || {}).map((indu, index) => (
                                <div key={index}>
                                    <span className='font-montserrat text-heading-color font-demibold text-xl text-lx'>{indu[0]}</span>: <span className='font-montserrat text-text-color text-xl font-normal leading-loose'>{indu[1]}</span>
                                </div>
                            ))}
                            </div>
                        </div>
